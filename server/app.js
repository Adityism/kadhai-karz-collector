// Send a real email (not SMS)
app.post('/email', (req, res) => {
  const { to, subject, message } = req.body;
  if (!to || !subject || !message) {
    res.send({ success: false, message: 'to, subject, and message parameters are required.' });
    return;
  }
  text.sendEmail(to, subject, message, (err, info) => {
    if (err) {
      res.send({ success: false, message: `Email send failed: ${err.message}` });
    } else {
